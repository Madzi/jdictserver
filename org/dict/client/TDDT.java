...
public class TDDT extends JFrame {
	...
	IDictEngine engine;
	java.util.Vector<DatabaseConfiguration> currentDatabases = new java.util.Vector<DatabaseConfiguration>();
	JTextArea fDisplay;
	// JFrame fFrame;
	...
	Hashtable<JTextComponent, KeyListener> vietKeys = new Hashtable<JTextComponent, KeyListener>();
	...
	protected JToolBar createToolBar() {
		...
		toolBar.add(new JButton("Clip"));
		toolBar.add(new JButton("History"));
		toolBar.add(new JButton("Back"));
	...
	private void databaseChanged() {
		...
		DatabaseConfiguration dc = currentDatabases.elementAt(k);
		IDatabase myDB = null;
	...
	private void saveSettings() {
		try {
			...
			for (int i = 0; i < currentDatabases.size(); i++) {
				DatabaseConfiguration dc = currentDatabases.elementAt(i);
	...
	private void showMatches(IAnswer[] a, boolean slide) {
		...
		for (int i = 0; i < a.length; i++) {
			...
			/*int count = matches.getLastVisibleIndex() - matches.getFirstVisibleIndex();*/
			/*
			 * for (int k = 0; k < n.length; k++) { if (n[k].getPosition() >=
			 * pos) { model.addElement(n[k].getKey()); } }
			 */
			if (slide) {
